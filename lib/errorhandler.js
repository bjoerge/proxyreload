...
function errorhandler(error, req, res, next) {
  ...
  function showErr() {
    ...
    var linkedStack = trace.map(function(callSite) {
      var loc = [callSite.lineNumber, callSite.columnNumber].filter(Boolean).join(":");
      var functionName = callSite.functionName || '<anonymous>';
      return '<span class="functionName">'+escape(functionName)+'</span>'+'(<span class="fileref">'+callSite.fileName+":"+loc+'</span>)';
    }).join("\n at ");
    ...
    getContext(trace, function(err, context) {
      ...
      res.status(502).send(compiledTemplate({
